...
function getPage() {
  ...
  makeStructure();
  fetch('../../json/codes.json').then(function (response) {
...
function setCode(page) {
  var des = '<div class="card-panel  blue-grey darken-1 white-text">\n                        <span class="card-title">\n                            <h4>Description</h4>\n                        </span>\n                        <div class="divider"></div>\n                        <span id="des" class="flow-text">' + page.title + ', ' + page.description + '</span>\n                    </div>';
  $("#des").html(des);
  var specs = '<div class="card-panel grey lighten-3 bullet">\n                        <span class="card-title">\n                            <h4>Specifications</h4>\n                        </span>\n                        <div class="divider"></div>\n                        <div id="specli"></div>\n                    </div>';
  $("#specs").html(specs);
...
function setSpecs(specs, page) {
  fetch('../../json/specifications.json').then(function (response) {
    return response.json();
  }).then(function (data) {
...
function setImages(images, title, height) {
  var image = '' + images.map(function (image) {
...
function codeTable(page) {
  var table = '\n        <div class="card padding">\n            <table class="striped highlight centered">\n                <thead>\n                <tr>\n                    <th>Cabinet Widths</th>\n                    <th>Order Codes</th>\n                </tr>\n                </thead>\n                <tbody id="tbody">                \n                    ' + page.widths.map(function (code) {
    return '<tr><td>' + code + '"</td><td class="ordercode">' + page.root + code + page.height + '</td></tr>';
  }).join('') + '   \n                </tbody>\n            </table>\n            ' + additional(page) + '\n        </div>\n        ';
  $("#codes").html(table);
}
...
function setActions(page) {
  ...
  fetch('../../json/' + cat + '.json').then(function (response) {
    return response.json();
  }).then(function (data) {
...
function makeActions(card) {
  var action = '' + card.attached.map(function (a) {
    return '<li class="waves-effect waves-light"><a class="white-text" href="./' + a.link + '.html?code=' + card.code + a.height + '"><i class="material-icons">art_track</i>' + a.height + '" high</a></li>';
  }).join('');
  return action;
}

function setDim(page) {
  fetch('../../json/iwhd.json').then(function (response) {
    return response.json();
  }).then(function (data) {
    ...
    var n = '<ul><b>Dimensional adjustments</b>:' + dim.map(function (iwhd) {
      return '<li class="second"><i class="material-icons">tune</i> ' + iwhd.title + ' - ' + iwhd.content + '</li>';
    }).join('') + '</ul>';
    // console.log(n);
    $("#dim").html(n);
  }).catch(function (err) {
...
function setOptions(page) {
  fetch('../../json/addons.json').then(function (response) {
    return response.json();
  }).then(function (data) {
...
function additional(page) {
  ...
  var add = '<br><div class="divider"></div><br>\n        <table class="striped highlight centered">\n            <thead><tr>\n            ' + page.additional.header.map(function (h) {
    return '<th>' + h + '</th>';
  }).join('') + '\n            </tr></thead>\n            <tbody id="tbody">\n            ' + page.additional.rows.map(function (r) {
    return '<tr><td>' + r.cw + '</td><td>' + r.fw + '</td><td>' + r.l + '</td><td>' + r.dw + '</td></tr>';
