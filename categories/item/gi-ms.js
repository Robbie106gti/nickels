...
function getPage() {
        ...
        fetch('../../json/general information.json').then(function (response) {
    ...
    function insertEmployees(page) {
        ...
        var employees = '' + people.map(function (p) {
    ...
    function setContact(page) {
        ...
        var social = '\n        <div class="card-panel grey lighten-5 z-depth-1">\n            <div class="row valign-wrapper">\n                <div class="col s12">\n                    <h4>Other ways to connect with us:</h4>\n                    <ul>\n                        ' + page.social.map(function (s) {
            return '<li>' + s.name + ' <a href="' + s.link + s.email + '">' + s.content + '</a></li>';
        }).join('') + '\n                    </ul>\n                </div>\n            </div>\n        </div>';
    ...
    function setDes(page) {
        var topic = '<h5 id="subHeader"">' + page.description + '</h5><div class="divider"></div>';
        $("#des").html(topic);
    }
    
    function setPar(page) {
        var para = '' + page.paragraphs.map(function (n) {
    ...
    function setimages(page) {
        var imag = '' + page.images.map(function (image) {
    ...
    function imageTitle(image) {
        ...
        var title = '<span class="card-title black-text"><b>' + image.title + '</b></span>';
        return title;
    }
