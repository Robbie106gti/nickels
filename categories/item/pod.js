...
function setGI(information) {
  const cat = `<h1 id="topic">${information.title}</h1><h5>${
        ...
        spec => `
            <div class="card horizontal blue-grey lighten-5">
                <div class="detail-card">
                    <span><img class="detail-image materialboxed" src="${
                      spec.image
                    }"></span>
                    <span><img class="detail-image materialboxed" src="${
                      spec.image2
                ...
                <div class="card-content">
                    <div class="collection">
                        ...
                        <div class="collection-item blue-grey-text text-darken-4"><b><ul><li>Code</b>: <span class="ordercode" >${
                          spec.title
                        }</span></li></ul></div>
                    </div>
                </div>
...
function getLinks(tags) {
  if (!tags) return;
  const keys = `${tags
    .map(a => `<a id="${a.title}" href="${a.link}">${a.title}</a><br>`)
    .join('')}`;
  return keys;
}
