...
function getPage() {
    ...
    function videos(page) {
        var videos = page.videos.map(function (video) {
    ...
    function cardWithout(video) {
        var card = '<div class="card ' + edge + '">\n                        <div class="video-container">\n                            <iframe src="https://player.vimeo.com/video/' + video.video + '" width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>\n                        </div>\n                        <div class="card-content">\n                            <span class="card-title grey-text text-darken-4">' + video.title + '</span>\n                            ' + getTags(video.tags) + '\n                        </div>\n                    </div>';
        return card;
    }
    ...
    function setImages(page) {
        ...
        dbx.filesListFolder({ path: '/nickels kitchens' }).then(function (response) {
            console.log(response);
            var images = response.entries.map(function (image) {
